...
function CDOTA_BaseNPC:RefreshAllCooldowns(bItems)
    for i = 0, self:GetAbilityCount() - 1 do
        ...
        if ability and ( ability.IsRefreshable == nil or ability:IsRefreshable() ) then
			ability:Refresh()
	...
	if bItems then
		for i=0, 5, 1 do
			...
			if current_item ~= nil and ( current_item.IsRefreshable == nil or current_item:IsRefreshable() ) then
