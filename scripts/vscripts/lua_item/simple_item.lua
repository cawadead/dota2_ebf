...
function refresher( keys )
	...
	local refresher_shared = {["item_octarine_core5"] = true,["item_asura_core"] = true,["item_refresher"] = true}
		...
		for _, unit in ipairs( caster:FindAllUnitsInRadius( caster:GetAbsOrigin(), currentRadius ) ) do
			if not unitsToHit[unit:entindex()] then
				...
				if unit:IsSameTeam( caster ) then
					item:ApplyDataDrivenModifier(caster, unit, "bahamutImmunity", { duration = duration })
				else
...
function Midas_OnHit(keys)
    ...
    local bonus_gold = math.ceil(damage ^ 0.08 + 2) 
    local ID = 0
    if GetMapName() == "epic_boss_fight_impossible" or GetMapName() == "epic_boss_fight_challenger" then
        ...
        for _,unit in pairs ( Entities:FindAllByName( "npc_dota_hero*")) do
            if GetMapName() == "epic_boss_fight_impossible" or GetMapName() == "epic_boss_fight_challenger" then
                if not unit:IsIllusion() and simple_item.midas_gold_on_round <= simple_item._round*150 then
					...
					print( bonus_gold, end_bonus_gold )
...
function Midas2_OnHit(keys)
    ...
    local player = PlayerResource:GetPlayer( caster:GetPlayerID() )
    local damage = keys.damage_on_hit
    local bonus_gold = math.floor(damage ^ 0.14 + 4 )
    local ID = 0
    if GetMapName() == "epic_boss_fight_impossible" or GetMapName() == "epic_boss_fight_challenger" then
                if simple_item.midas_gold_on_round <= simple_item._round*150 and item:IsCooldownReady() and not caster:IsIllusion() then
                    simple_item:midas_gold(bonus_gold)
...
function Midas3_OnHit(keys)
    ...
    local player = PlayerResource:GetPlayer( caster:GetPlayerID() )
    local damage = keys.damage_on_hit
    local bonus_gold = math.floor( damage ^ 0.16 + 6 )
    local ID = 0
    if GetMapName() == "epic_boss_fight_impossible" or GetMapName() == "epic_boss_fight_challenger" then
                if simple_item.midas_gold_on_round <= simple_item._round*200 and item:IsCooldownReady() and not caster:IsIllusion() then
                    simple_item:midas_gold(bonus_gold)
                end
    elseif item:IsCooldownReady() and not caster:IsIllusion() then
        simple_item:midas_gold(bonus_gold)
...
function Pierce_Splash(keys)
	...
	local actualDamage = damage + ( damage / target:GetPhysicalArmorMultiplier() ) * percent_p
	
	print(actualDamage, damage, ( damage / target:GetPhysicalArmorMultiplier() ), ( damage / target:GetPhysicalArmorMultiplier() ) * percent_p )
	...
	for _,unit in ipairs( caster:FindEnemyUnitsInRadius( target:GetAbsOrigin(), radius ) ) do
		if unit ~= target then
			local damageTable = {
				victim = unit,
				attacker = caster,
				damage = actualDamage*percent/100,
				damage_type = DAMAGE_TYPE_PHYSICAL,
				ability = item,
				damage_flags = DOTA_DAMAGE_FLAG_NO_SPELL_AMPLIFICATION
			}
			...
			item:DealDamage( caster, target, damage * percent_p, {damage_type = DAMAGE_TYPE_PURE, damage_flags = DOTA_DAMAGE_FLAG_NO_SPELL_AMPLIFICATION} )
		end
	end
end
