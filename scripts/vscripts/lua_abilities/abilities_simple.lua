        ...
        if not caster.dead or not caster:IsAlive() then
			if not caster.dead then
				...
				Timers:CreateTimer(1.0,function()
					for _,unit in pairs ( Entities:FindAllByName( "npc_dota_creature")) do
						if unit:GetTeamNumber() == DOTA_TEAM_BADGUYS and not ( unit:GetUnitName() == "npc_dota_boss36" or unit:GetUnitName() == "npc_dota_boss36_guardian" ) then
							print("get murdered")
							unit:ForceKill(true)
						end
					end
        ...
        Timers:CreateTimer(timer,function()
            ...
            if vDiff:Length2D() < Death_range then
                ...
                Timers:CreateTimer(function()
					...
					if unit:IsAlive() then
						ability:DealDamage( caster, unit, unit:GetMaxHealth() * 0.5 * FrameTime(), {damage_type = DAMAGE_TYPE_MAGICAL, damage_flags = DOTA_DAMAGE_FLAG_NO_SPELL_AMPLIFICATION + DOTA_DAMAGE_FLAG_HPLOSS + DOTA_DAMAGE_FLAG_BYPASSES_INVULNERABILITY } )
					end
					if unit:GetHealth() == 1 then
						unit:ForceKill( false )
					end
					if deathDuration > 0 then
						deathDuration = deathDuration - FrameTime()
						return 0.01
					else
						unit.NoTombStone = false
					end
...
function hell_tempest_boss_start( event )
	...
	ParticleManager:SetParticleControl( caster.tempestFX2, 1, Vector( 350, 350, 350 ) )
	ParticleManager:SetParticleControl( caster.tempestFX2, 2, Vector( 350, 350, 350 ) )
end
