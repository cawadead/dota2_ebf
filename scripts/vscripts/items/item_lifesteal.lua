...
function modifier_item_lifesteal_ebf_aura:OnTakeDamage(params)
	if params.attacker == self:GetParent() and params.damage_category == DOTA_DAMAGE_CATEGORY_ATTACK then
		...
		params.attacker:HealWithParams( lifesteal, nil, true, true, self:GetAbility(), false )
...
function modifier_item_lifestealbooster_ebf:OnCreated()
	...
	if IsServer() then
		self:GetParent():AddNewModifier( self:GetParent(), self:GetAbility(), "healthBooster", {} )
	end
end
...
function modifier_item_lifestealtank_ebf_aura:OnTakeDamage(params)
	if params.attacker:IsSameTeam( self:GetParent() ) and params.attacker == self:GetParent() and params.damage_category == DOTA_DAMAGE_CATEGORY_ATTACK then
		local EHPMult = self:GetParent().EHP_MULT or 1
		
		local preHP = params.attacker:GetHealth()
		params.attacker:HealWithParams( params.damage * self.lifesteal_aura * EHPMult, nil, true, true, self:GetAbility(), false )
		...
		if params.attacker ~= self:GetCaster() then
			...
			self:GetCaster():HealWithParams( params.damage * self.lifesteal_shared * EHPMult, nil, true, true, self:GetAbility(), false )
