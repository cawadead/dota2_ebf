...
function modifier_item_mjollnir_ebf:GetModifierPreAttack_CriticalStrike(params)
...
function modifier_item_mjollnir_rot_ebf:GetModifierIncomingDamage_Percentage(params)
	if IsServer() and params.inflictor then
		local dmgIncrease = self.spell_amp
		local EHPMult = self:GetParent().EHP_MULT or 1
		local damagemult = 1/EHPMult * dmgIncrease
